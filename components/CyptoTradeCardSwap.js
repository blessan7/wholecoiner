...
const useTokenSearch = () => {
  ...
  // Function to transform Jupiter API token to your Token interface
  ...
  const transformBackendToken = (backendToken) => {
    return {
      ...
      fdv: backendToken.fdv || null,
      mcap: backendToken.mcap || null,
      liquidity: backendToken.liquidity || null,
      organicScore: backendToken.organicScore || null,
      organicScoreLabel: backendToken.organicScoreLabel || null,
      tags: backendToken.tags || [],
      cexes: backendToken.cexes || [],
      source: "backend",
    };
  };

  // Function to search Jupiter API
  const searchJupiterTokens = async (query, limit = 50) => {
    try {
      ...
      const filteredTokens = jupiterTokens.filter(
        (token) => token.isVerified === true
      );

      const transformedTokens = filteredTokens.map(transformJupiterToken);
    ...
    async (query = "", limit = 50) => {
      ...
      try {
        ...
        try {
          ...
          const rawBackendTokens = backendResponse.data.tokens || [];
          backendTokens = rawBackendTokens.map(transformBackendToken);
        } catch (error) {
          backendError = error;
        }

        // Only search Jupiter if we have a query AND it's worth supplementing
          ...
          try {
            ...
            if (jupiterTokens.length > 0) {
              const backendAddresses = new Set(
                backendTokens.map((token) => token.address.toLowerCase())
              );

              const uniqueJupiterTokens = jupiterTokens.filter(
                (jupiterToken) =>
                  !backendAddresses.has(jupiterToken.address.toLowerCase())
              );
...
// MAIN COMPONENT
// ========================
const CryptoTradeCardSwap = ({ userTokens, action }) => {
  // ========================
  // STATE
  // ========================
  const [tokens, setTokens] = useState(userTokens);
  const [selectedCrypto1, setSelectedCrypto1] = useState(userTokens[0]);
  const [selectedCrypto2, setSelectedCrypto2] = useState(null);
  const [selectedSlippage, setSelectedSlippage] = useState(slippageOptions[2]);
  ...
  // CUSTOM HOOK USAGE
  // ========================
  const { searchResults, isSearching, searchTokens } = useTokenSearch();
  ...
  async function getSolBalance(userPublicKey, connection) {
    const publicKey =
  ...
  const fetchTokenPrice = async (address, symbol) => {
    try {
      ...
      if (priceDataWithVsToken) {
        return priceDataWithVsToken[address]?.usdPrice;
      } else {
  ...
  const fetchSelectedTokenBalance = async () => {
    if (!wallets[0]?.address || !selectedCrypto1) return;
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          const solBalance = await getSolBalance(userPublicKey, connection);
          balance = solBalance / 1000000000;
        } else {
        ...
        console.log(
          `Error fetching balance for ${selectedCrypto1.symbol}:`,
          error
        );
        const updatedSelectedToken = {
          ...selectedCrypto1,
  ...
  async function getSwapInfo() {
    ...
    const amount = BigInt(
      Math.floor(Number(depositToken) * 10 ** selectedCrypto1.decimals)
    );
    ...
    try {
      ...
      if (selectedCrypto1.symbol !== "SOL") {
        mintInfoInput = await connection.getAccountInfo(
          new PublicKey(selectedCrypto1.address)
        );
        isToken2022Input = mintInfoInput?.owner.equals(TOKEN_2022_PROGRAM_ID);
      }
      if (selectedCrypto2.symbol !== "SOL") {
        mintInfoOutput = await connection.getAccountInfo(
      ...
      const eitherMintIsNotSOLAndNotToken22 =
        isToken2022Input === false || isToken2022Output === false;

      console.log({ eitherMintIsNotSOLAndNotToken22 });
      ...
      if (
        ...
        toast.error(
          "No route found for trade. Please try some other combination"
        );
        return;
      }
      if (
        ...
        toast.error("Token not tradeable");
        return;
      }
      if (
        quoteResponse.error &&
        quoteResponse.errorCode === "ROUTE_PLAN_DOES_NOT_CONSUME_ALL_THE_AMOUNT"
      ) {
        toast.error("Try reducing input amount, trade value too large");
        return;
      }

      if (quoteResponse.error) {
        await axios.post(`${API_URL}/api/error`, {
          data: quoteResponse.error,
        });
        toast.error(
          "Something went wrong while fetching trade route. Please try again."
        );
        return;
      }
      ...
      setReceiveToken(
        (
          quoteResponse.outAmount / Math.pow(10, selectedCrypto2.decimals)
        ).toString()
      );
  ...
  async function getUserToken(address) {
    try {
      setIsLoadingTokens(true);
      const tokens = await getTokens(new PublicKey(address), connection);
      setTokens(tokens);
    } catch (error) {
      console.log(error);
  ...
  const handleKeypadPress = async (value) => {
    ...
    const currentToken =
      activeInput === "send" ? selectedCrypto1 : selectedCrypto2;

    if (value === "CLEAR") {
      setValue("0");
    } else if (value === "MAX" || value === "75%" || value === "50%") {
      if (!currentToken || !wallets[0]?.address) {
        setValue("0");
        toast.error("No token selected or wallet not connected");
        return;
      }
      ...
      try {
        ...
        if (
          ...
          balance = await getFormattedTokenBalance(
            wallets[0].address,
        ...
        if (balance) {
          ...
          if (value === "MAX") {
            if (selectedCrypto1.symbol === "SOL") {
              if (balance < 0.00211) {
                toast.error("SOL balance must be more than 0.00211");
              } else {
          ...
          setValue("0");
          toast.error("Token balance is zero");
        }
      } catch (error) {
  ...
  async function swapViaDex() {
    if (isLoading) return;

    if (!selectedCrypto1) {
      toast.error(
        `Please select token to ${action === "buy" ? "pay" : "sell"}`
      );
      return;
    }
    ...
    if (
      ...
      receiveToken === "" ||
      Number(receiveToken) === 0
    ) {
      toast.error("Tokens can't be Zero");
      return;
    }

    setIsLoading(true);
    try {
      let solBalance = await getSolBalance(wallets[0].address, connection);
      solBalance = solBalance / 1000000000;

      if (solBalance < 0.000105) {
        toast.error(
          "SOL balance must be more than 0.000105 to cover transaction fees"
        );
        return;
      }
      ...
      if (selectedCrypto1?.symbol === "SOL") {
        if (solBalance < 0.00211) {
          toast.error(
            "SOL balance must be more than 0.00211 to cover account maintenance and transaction fees"
          );
          return;
        }
        if (solBalance < Number(depositToken)) {
          toast.error(`you have ${solBalance} SOL balance`);
          setIsLoading(false);
          return;
        }
      } else {
        ...
        console.log({ tokBalance, depositToken });

        if (tokBalance < Number(depositToken)) {
          toast.error(
            `you have ${tokBalance} ${
              selectedCrypto1.symbol || "Token"
            } balance`
          );
          setIsLoading(false);
          return;
      ...
      if (!userAccountInfo && !isOutputTokenSol) {
        const res = await axios.get(
          `${API_URL}/api/user/ata/${selectedCrypto2.address}`,
          {
      ...
      console.log(JSON.stringify(swapBody, null, 2));
      ...
      await wallets[0].signTransaction(transaction);
      ...
      try {
        ...
        if (jupErr) {
          ...
          let errorMessage =
            "Kindly ensure your balance has at least 0.003 additional SOL before proceeding, or try again after some time.";

          if (
            JSON.stringify(jupErr).includes("6001") ||
            JSON.stringify(jupErr).includes("6017")
          ) {
            errorMessage =
              "Your trade did not complete because the market price changed before your order was processed. No funds were lost";
          }
        ...
        if (isBlockheightError) {
          console.log(
            "Transaction confirmation failed due to blockheight exceeded. Fetching new quote..."
          );
          try {
            await getSwapInfo();
            shouldResetTokens = false;
            toast.error(
              "Transaction expired. New quote fetched - please try again."
            );
            setIsLoading(false);
            return;
          } catch (quoteError) {
            console.log("Error fetching new quote:", quoteError);
            toast.error("Transaction expired. Please refresh and try again.");
          }
          setIsLoading(false);
          return;
        }
      ...
      await axios.post(
        `${API_URL}/api/dex`,
        {
          ...
          fromTokenMintAddress: selectedCrypto1.address,
          toTokenMintAddress: selectedCrypto2.address,
          fromTokenMintSymbol: selectedCrypto1.symbol || selectedCrypto1.name,
          toTokenMintSymbol: selectedCrypto2.symbol || selectedCrypto2.name,
          fromTokenAmount: depositToken,
          toTokenAmount: receiveToken,
        },
        {
          headers: { Authorization: `Bearer ${accessToken}` },
          timeout: 15000,
      ...
      if (isBlockheightError) {
        console.log(
          "Transaction expired due to blockheight exceeded. Fetching new quote..."
        );
        try {
          await getSwapInfo();
          shouldResetTokens = false;
  ...
  useEffect(() => {
    async function updateTokenPrices() {
      if (selectedCrypto2?.symbol) {
        const price = await fetchTokenPrice(
          selectedCrypto2.address,
          selectedCrypto2.symbol
        );
  ...
  useEffect(() => {
    if (wallets && wallets[0]?.address) {
      getUserToken(new PublicKey(wallets[0].address));
    }
  }, [wallets]);

  useEffect(() => {
    if (selectedCrypto1 && wallets[0]?.address) {
      fetchSelectedTokenBalance();
    }
  }, [selectedCrypto1?.address, wallets[0]?.address]);
    ...
    <div className="flex-1 bg-[#0A1E1E] p-2 min-h-screen lg:p-10  relative">
      ...
      <div className="flex items-center justify-between mb-7">
        <div className="flex items-center gap-2">
          ...
          >
            <ChevronRight
              className="rotate-180 text-[#FFB217]"
              width={18}
              height={18}
            />
            <span className="text-[#FFB217] text-xs font-normal mt-0.5">
              BACK
            </span>
          </button>
        </div>
      ...
      <div className="w-full bg-[#0c2626] rounded-[32px] p-6 font-['Montserrat']">
        ...
        <div className="mb-6">
              ...
              <div className="flex flex-row items-center gap-2 max-w-[200px]">
                ...
                <div className="flex flex-col flex-1">
                  ...
                  <span className="text-[#e0cdcd] text-xs italic">
                    {selectedCrypto1?.formattedBalance?.toFixed(6) ||
                      "0.000000"}{" "}
                    {selectedCrypto1?.symbol || ""}
                  </span>
                </div>
          ...
          <div className="flex flex-row justify-between mt-4">
            <button
              onClick={() => openKeypad("send")}
              className="flex flex-col gap-1"
            >
              <span className="text-white text-4xl font-bold">
                {formatDisplayValue(depositToken)}
              </span>
              <span className="text-[#FFB217] text-sm">
                ≈ ${depositUsdValue}
              </span>
            </button>
            <button
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-row justify-between mt-4 relative">
            <div className="flex flex-col gap-1">
              <span className="text-[#248368] text-4xl font-bold">
                {receiveToken}
              </span>
              <span className="text-[#FFB217] text-sm">
                ≈ ${receiveUsdValue}
                ...
                {showTooltip && (
                  <div className="absolute bottom-full right-0 mb-2 w-64 bg-[#36676E] rounded-2xl rounded-br-none p-4 z-50">
                    <p className="text-white text-sm leading-6">
                      DISCLAIMER: Before purchasing any token, always verify its
                      logo, symbol, and rating. Scammers often create lookalike
                      tokens that mimic the name or branding of legitimate
                      projects. Ensure you're interacting with the correct
                      token.
                    </p>
                  </div>
                )}
          ...
          <div className="fixed inset-0 bg-[#0c2626] z-50 flex flex-col">
            ...
            <div className="flex justify-between items-center px-5 py-4 border-b border-[#174545]">
              ...
              <button
                ...
                className="text-[#FFB217] text-2xl font-bold"
              >
                ✕
              </button>
            </div>
            ...
            {isLoadingTokens && (
              <div className="flex justify-center p-4">
                <svg
                  className="animate-spin h-6 w-6 text-[#FFB217]"
                  viewBox="0 0 24 24"
                >
                  <circle
                    className="opacity-25"
            ...
            {tokenModalType === "send" ? (
              <>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4">
                  <h4 className="text-[#FFB217] text-base font-semibold mb-3">
                    Your Tokens
                  </h4>
                        ...
                        {searchResults.map((item) => (
                          ...
                          >
                            ...
                            <div className="flex-1 ml-2 text-left">
                              ...
                              {item.source === "backend" &&
                                item.organicScoreLabel && (
                                  <span className="text-[#4CAF50] text-[11px]">
                                    Score: {item.organicScoreLabel}
                                  </span>
                                )}
                            </div>
                          </button>
            ...
            <div className="bg-[#0c2626] rounded-t-3xl w-full p-5 pb-10">
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex-1">
                  <p className="text-[#FFB217] text-sm font-bold mb-2">
                    Enter Amount
                  </p>
                  <div className="flex items-center gap-2">
                    <span className="text-white text-4xl font-bold">
                  ...
                  <p className="text-[#FFB217] text-base mt-1">
                    ${getCurrentUsdValue()}
                  </p>
                  {activeInput === "send" && selectedCrypto1 && (
                    <p className="text-gray-500 text-sm italic mt-1">
                      Balance: {selectedCrypto1.formattedBalance || "0.000000"}{" "}
                      {selectedCrypto1.symbol || ""}
                    </p>
                  )}
                </div>
                <button
                  ...
                  className="text-[#FFB217] text-xl font-bold p-2"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="flex flex-wrap justify-between">
                ...
                <button
                  ...
                  className="w-[22%] h-12 rounded-xl mb-3 text-white text-2xl font-semibold"
                >
                  1
                </button>
                <button
                ...
                <button
                  ...
                  className="w-[22%] h-12 bg-[#124D4B] rounded-xl mb-3 text-white text-sm font-semibold"
                >
                  CLEAR
                </button>
                <button
                ...
                <button
                  ...
                  className="w-[22%] h-12 bg-[#124D4B] rounded-xl mb-3 text-white text-2xl font-semibold"
                >
                  0
                </button>
                <button
...
export default CryptoTradeCardSwap;